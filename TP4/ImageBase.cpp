...
ImageBase* ImageBase::toYCbCr() {
	...
	for (int i=0; i<nTaille; i+=3) {
		Color current = Color(i, this->data);
		current.toYCbCr();
		imageRes->dataD[i] = current.r;
		imageRes->dataD[i+1] = current.g;
		imageRes->dataD[i+2] = current.b;
	}
	return imageRes;
}

ImageBase* ImageBase::toRGB() {
	ImageBase* imageRes = new ImageBase(this->getWidth(), this->getHeight(),
										true);
	cout << "testRGB" << endl;
	for (int i=0; i<nTaille; i+=3) {
		Color current = Color(i, this->dataD);
		current.toRGB();
		imageRes->data[i] = current.r;
		imageRes->data[i+1] = current.g;
		imageRes->data[i+2] = current.b;
	}
	return imageRes;
}

ImageBase* ImageBase::reduceSpace() {
	ImageBase* imageRes = new ImageBase(this->getWidth(), this->getHeight(),
										true);
	...
	for (int i = 0; i < height; i++) {
		for (int j = 0; j < width ; j++) {
			...
			if (i%2 == 0 && j%2 == 0) {
				imageRes->dataD[index] = this->dataD[index];
				imageRes->dataD[index+1] = this->dataD[index+1];
				imageRes->dataD[index+2] = this->dataD[index+2];
			} else if (i%2 == 0) {
				int otherPlanIndex = (i * width + j-1) * 3;
				imageRes->dataD[index] = this->dataD[otherPlanIndex];
				imageRes->dataD[index+1] = this->dataD[index+1];
				imageRes->dataD[index+2] = this->dataD[index+2];
			} else if (j%2 == 0) {
				int otherPlanIndex = ((i - 1) * width + j) * 3;
				imageRes->dataD[index] = this->dataD[otherPlanIndex];
				imageRes->dataD[index+1] = this->dataD[index+1];
				imageRes->dataD[index+2] = this->dataD[index+2];
			} else {
				int otherPlanIndex = ((i -1) * width + j -1) * 3;
				imageRes->dataD[index] = this->dataD[otherPlanIndex];
				imageRes->dataD[index+1] = this->dataD[index+1];
				imageRes->dataD[index+2] = this->dataD[index+2];
			}
...
tuple<string, HuffmanTree> ImageBase::huffmanCompress() {
	string s(reinterpret_cast<char*>(this->data), nTaille);
	...
	cout << "encoded (compression ratio: "
		<< s.size() << "/" << data.size() << " or "
		<< (float) s.size() / (float) data.size() << "):" << endl;
	return make_tuple(data,*htree);
}

unsigned char* ImageBase::huffmanDecode(string data, HuffmanTree htree) {
	code_t t1 = string_to_bitvec(data);
	string s1 = decode(t1, &htree);
	const unsigned char* res = reinterpret_cast<const unsigned char *>(s1.c_str());
	return const_cast<unsigned char*>(res);
}

vector<unsigned char> ImageBase::generateKey(int key) {
	srand(key);
	vector<unsigned char> res;
	for (int i = 0; i<nTaille; i++) {
		res.push_back(rand() / (RAND_MAX / 255));
	}
	return res;
}

ImageBase* ImageBase::XOR(int key) {
	ImageBase *imageRes = new ImageBase(this->getWidth(), this->getHeight(),
										false);
	vector<unsigned char> seq = this->generateKey(key);
	for (int i = 0; i<nTaille; i++) {
		imageRes->data[i] = (this->data[i] ^ seq[i]);
	}
	return imageRes;
}

float ImageBase::entropy() {
	calcHistogram();
	float entropy = 0.0;
	for (int i = 0; i<256; i++) {
		float p = (float) histogramRed[i] / (float) nTaille;
		if (histogramRed[i] == 0) continue;
		entropy += (float)p * (float) log2(p);
	}
	return -entropy;
}

#include <optional>
optional<ImageBase*> ImageBase::bruteforce() {
	ImageBase *imageRes = new ImageBase(this->getWidth(), this->getHeight(),
										false);
	for (int i=0; i<=100; i++) {
		imageRes = this->XOR(i);
		if (imageRes->entropy() < 7.98) return imageRes;
	}
	return nullopt;
}

ImageBase* ImageBase::diff(const ImageBase& other) {
	ImageBase *imageRes = new ImageBase(this->getWidth(), this->getHeight(),
										false);
	for (int i = 0; i<nTaille; i++) {
		imageRes->data[i] = abs(other.data[i] - this->data[i]);
	}
	return imageRes;
}
